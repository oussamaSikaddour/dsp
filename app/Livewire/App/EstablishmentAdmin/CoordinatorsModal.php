...
use App\Livewire\Forms\App\Coordinator\ManageForm;
use App\Models\Role;
use App\Models\User;
...
class CoordinatorsModal extends Component
{
    ...
    /**
     * =====================================================
     * AVAILABLE USERS (CAN BECOME COORDINATORS)
     * =====================================================
     */
    #[Computed]
    ...
    public function coordinators()
    {
        return User::query()
            ->where('establishment_id', $this->establishmentId)
            ->whereHas('roles', fn($q) => $q->where('slug', 'service_coordinator'))
    ...
    private function refreshPersonalOptions(): void
    {
        ...
        $this->personalOptions = $this->populateSelectorOption(
            ...
            'id',
            'full_name',
            __('selectors.default.coordinators')
        );
    }
    ...
    public function openManageCoordDialog(array $coord): void
    {
        $key = 'roles.detach.coordinator';
    ...
    public function removeCoordFunction(User $coord): void
    {
        try {
            DB::transaction(function () use ($coord) {

                if ($role = Role::coordinator()) {
                    $coord->roles()->detach($role->id);
                }
            ...
            Log::error('Error removing coordinator', [
    ...
    {
        return view('livewire.app.establishment-admin.coordinators-modal');
    }
}
