...
public function resolveRouteName(string $route): ?string
{
    ...
    if (defined(RoutesNames::class . '::' . $normalizedRoute)) {
        return constant(
            RoutesNames::class . '::' . $normalizedRoute
        )->value;
    }
...
public function setSlideOrder(array $data, $slider, $slide = null)
{
    ...
    protected function buildTreeData(
        ...
        string $childrenRelation = 'childrenRecursive',
        ...
        return $items->map(function ($item) use (
            ...
            $children = data_get($item, $childrenRelation);

            $node = [
                'id' => (string) Str::uuid(),
                'label' => data_get($item, $labelAttribute),
                'value' => data_get($item, $valueAttribute),
            ...
            foreach ($extraAttributes as $attribute) {
                $node[$attribute] = data_get($item, $attribute);
            }
